...
| Surface | Local | Cloud |
| --- | --- | --- |
| Live camera / stills | `npm run dev` in `frontend/` | Vercel production (OpenCV.js + YuNet) |
| YOLOX objects | `uvicorn` in `backend/` | Optional container + `API_URL` |
| Both | `docker compose -f infra/docker-compose.yml up` | Frontend on Vercel; backend only if you want objects |
